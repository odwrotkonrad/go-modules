...
func (Options) JSONSchema() *jsonschema.Schema {
	...
	s.Properties.Set("writeType", &jsonschema.Schema{
		Description: "how the rendered body lands: overwrite (default: header + body) | mergeUpsert (env KEY=VALUE union under the existing dest: a shell or secret value overwrites an existing key, a plain value keeps it, `| alwaysUpdate` / `| keepIfExisting` after the expression decide per line)",
