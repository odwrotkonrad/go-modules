...
func renderGroupIndex(dir string, node groupNode, level int, rel string) string {
	...
	if len(node.childRepos) > 0 {
		_ = indexIntroTpl.Execute(&b, struct{ Section, Label, Tree string }{heading(level), groupLabel(dir), renderGroupTree(dir)})
		b.WriteByte('\n')
	}
...
func renderGroupBody(dir string, node groupNode, level int, rel string) string {
	...
	for _, name := range node.childRepos {
		...
		b.WriteString(readPurpose(filepath.Join(dir, name), level+1))
		b.WriteByte('\n')
	}
	for _, name := range node.childSubgroups {
		if b.Len() > 0 {
...
func renderGroupTree(dir string) string {
	var b strings.Builder
...
func readPurpose(repoDir string, repoLevel int) string {
...
func containsRepo(dir string) bool {
	if isRepoDir(dir) {
		return true
	}
	entries, err := os.ReadDir(dir)
	...
	return slices.ContainsFunc(entries, func(e os.DirEntry) bool {
		return e.IsDir() && containsRepo(filepath.Join(dir, e.Name()))
	})
}
