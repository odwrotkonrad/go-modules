...
func splitTemplateLeaf(node templateNode, down templateInherited, globs *globSet, explicit *[]FileItem) error {
	...
	default:
		*explicit = append(*explicit, FileItem{
			Rel:   leaf.Source,
			Dests: mergeDestOptions(down.options, leaf.Dest),
			Ctx:   down.ctx,
			Perms: down.perms,
		})
	}
...
func checkTemplateGroup(node templateNode) error {
	if len(node.Dest) > 0 || node.DestRule != "" || node.glob != "" {
		return fmt.Errorf("renderTemplates group %q carries nested renderTemplates and a dest or glob: it is one or the other", node.Source)
	}
	return nil
}
...
func overlayRenderOptions(override, base render.Options) render.Options {
	if override.WriteType == "" {
		override.WriteType = base.WriteType
	}
	if !override.SkipAutoGeneratedHeader {
		override.SkipAutoGeneratedHeader = base.SkipAutoGeneratedHeader
	}
	if !override.RenderReferencedFiles {
