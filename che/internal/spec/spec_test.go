...
type wantSet struct {
	...
	Vars          map[string]map[string]string `yaml:"variables"`
...
func assertWant(t *testing.T, ops OperationRecipes, refs []ProfileSourceRecipe, w wantSet) {
	...
	for key, want := range w.Vars {
		item := findItem(ops, key)
		if !assert.NotNil(t, item, "variables: %q not resolved", key) {
			continue
		}
		assert.True(t, maps.Equal(item.Vars, want), "variables[%q] = %v, want %v", key, item.Vars, want)
	}
	...
	for i, opts := range w.TemplateOpts {
		...
		for j, o := range opts {
			got := optWant{
				...
				CommentPrefix:           dests[j].Options.CommentPrefix,
