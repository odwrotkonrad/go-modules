...
func assertWant(t *testing.T, res Resolved, w wantSet) {
	...
	for i, opts := range w.TemplateOpts {
		...
		for j, o := range opts {
			if dests[j].Options.WriteType != o.WriteType || dests[j].Options.SkipAutoGeneratedHeader != o.SkipAutoGeneratedHeader || dests[j].Options.RenderReferencedFiles != o.RenderReferencedFiles {
