...
func Schema() *jsonschema.Schema {
	...
	defs["ProfileRecipe"].Description = "one profile block: options self-describe eligibility, include.profiles compose refs in order (local scalars, sourced {url, ref, specDirPath, profileName, options, env, variables}), include adds, exclude filters last and wins"
...
func topIncludeSchema() *jsonschema.Schema {
	...
	o.Properties.Set("sources", &jsonschema.Schema{
		Description: "spec sources: a local <dir> scalar (absolute, relative, ~/, $VAR), or {url, ref, specDirPath, env, variables, optional}; a dir offers its che.export.yml first, then a plain che.yml",
		Type:        "array",
		Items:       scalarOr("local spec dir", entry),
	})
	return o
}
...
func (linkEntry) JSONSchema() *jsonschema.Schema {
	...
	o.Properties.Set("source", remoteSourceSchema("file or glob, workingDirectory-relative"))
...
func (copyNode) JSONSchema() *jsonschema.Schema {
	...
	leaf.Properties.Set("source", remoteSourceSchema("source path, any file, workingDirectory-relative, or remote ref git::<repo>[@<ref>]//<path> (explicit dest required); joined onto every enclosing group's source prefix"))
...
func (templateNode) JSONSchema() *jsonschema.Schema {
	...
	leaf.Properties.Set("source", remoteSourceSchema("source path (host sources workingDirectory-relative, repo-doc sources checkout-relative), or remote ref git::<repo>[@<ref>]//<path> (explicit dest required); joined onto every enclosing group's source prefix"))
	leaf.Properties.Set("dest", &jsonschema.Schema{OneOf: []*jsonschema.Schema{
		{
