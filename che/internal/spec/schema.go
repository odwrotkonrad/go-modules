...
func specsIncludeEntrySchema() *jsonschema.Schema {
	...
	entry.Properties.Set("optional", &jsonschema.Schema{
		Description: "local dir sources only: an absent dir is skipped with a warning instead of failing the load (a dir another profile renders first); never loaded when --profiles selects only this spec's profiles",
		Type:        "boolean",
	})
	return entry
}
