...
- `writeType`: `""` (overwrite, the default, autogen header plus body) |
  `mergeUpsert` (env `KEY=VALUE` union under existing dest, no header). Under
  `mergeUpsert` a `secret` or `shell` value overwrites its existing key and a
  plain value keeps it; `| alwaysUpdate` and `| keepIfExisting` after any
  expression decide per line, and piping a whole multi-line block through one
  marks every `KEY=VALUE` line in it, so `{{ localFile ".repo/upstream.env" |
