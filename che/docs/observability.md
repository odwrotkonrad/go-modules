...
che emits OTLP telemetry (metrics + mirrored logs) to a local collector, wired in `internal/telemetry`. Off by default: a nil provider is a no-op.
...
`telemetry.Config` knobs, driven by the `otel:` spec block and `CHE_OTEL_*` env (env wins). See the `otel` group in [che.yml schema](../assets/data/che.schema.json).
