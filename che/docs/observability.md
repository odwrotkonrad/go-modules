...
che emits OTLP telemetry (metrics plus mirrored logs) to a local collector, wired in `internal/telemetry`. Off by default, where a nil provider is a no-op.
...
Values observed at the emission sites in `internal/che`, not an enforced enum.
...
With `otel.traces` on, che emits spans on tracer `che`, nested `che run` > `prepare-specs` / `<command>` > `profile` > `<operation>` > external calls (`fetch-remote`, `run-script`). Counters record under the active span ctx, so metric exemplars link back to the trace.
...
With `otel.logs` on, the log bridge (`Telemetry.LogRecord`) mirrors each che log event as an OTLP record. Event name `<scope>.<action>`, message as body, event attributes (plus any skip reasons) as record attributes, level mapped to severity (`error` -> Error, `warn` -> Warn, `info` -> Info, `debug` -> Debug, `trace` -> Trace).

## Config

`telemetry.Config` knobs, driven by the `otel:` spec block and `CHE_OTEL_*` env (env wins). See the `otel` group in the [che.yml schema](../assets/data/che.schema.json).
