...
func serveLanguages(t *testing.T) *httptest.Server {
	t.Helper()
...
func TestResolveCacheDir(t *testing.T) {
	testyml.Eq(t, td, "testdata/spec/funcs/resolve_cache_dir.test.spec.yml", func(t *testing.T, c testyml.Case[string]) (string, error) {
		return ResolveCacheDir(), nil
	})
}

func setupFetch(t *testing.T, precondition string) string {
	t.Helper()
	switch precondition {
...
func TestFetchLanguages(t *testing.T) {
	testyml.Run(t, td, "testdata/spec/funcs/fetch_languages.test.spec.yml", func(t *testing.T, c testyml.Case[struct{}]) {
		url := setupFetch(t, c.Input.Args.String(t, 0))
		_, err := fetchLanguages(url)
		c.Expected.Check(t, err)
	})
}

func TestLoadTypeExtensions(t *testing.T) {
	testyml.Run(t, td, "testdata/spec/funcs/load_type_extensions.test.spec.yml", func(t *testing.T, c testyml.Case[struct{}]) {
		cache := t.TempDir()
		require.NoError(t, os.WriteFile(filepath.Join(cache, "languages.yml"), []byte(c.Input.Args.String(t, 0)), 0o644))
		t.Setenv("LINGUIST_CACHE_DIR", cache)
		_, err := LoadTypeExtensions("http://unused.invalid")
		c.Expected.Check(t, err)
	})
}
