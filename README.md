<!-- autogenerated using git::gitlab.com/konradodwrot/centralized/assets/prose/human@docs/che-scoped-variables//prose/repos/go-modules/README.md.ontoRepo.tpl -->
# go-modules
...
Go monorepo for all user Go modules: `che` (spec-driven dotfile loader, with the shared render engine and doc-rendering CLIs as its `render/` package tree), `get-os-open-files-with`, `get-term-open-files-with`, `lib` (config-loading and CLI-main helpers for the get-* CLIs). Each module keeps its own `go.mod` and release stream.

### Why It Exists

Four sibling repos duplicated toolchain, CI, conventions, release flow. One repo carries all of it: cross-module changes land in one MR, render-files folded into che, its only consumer.
